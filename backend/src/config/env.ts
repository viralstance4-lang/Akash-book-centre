...
const schema = z.object({
  ...
  // ── AWS S3 (print-order PDFs only — product images stay on Cloudinary) ──────
  /** Region of the S3 bucket, e.g. "us-east-1" */
  ...
  // Without these, print-order PDF uploads (s3.ts) would silently fail at
  // runtime for every order instead of failing loudly at startup.
  if (data.NODE_ENV === "production" && (!data.AWS_REGION || !data.AWS_ACCESS_KEY_ID || !data.AWS_SECRET_ACCESS_KEY || !data.AWS_S3_BUCKET)) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["AWS_S3_BUCKET"],
      message: "AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY and AWS_S3_BUCKET are all required when NODE_ENV=production (print-order PDF uploads go to S3)",
    });
  }
});
